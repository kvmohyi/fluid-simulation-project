...
void FluidSimulation::instantiateFromFile(string file) {
	...
	if(!inpfile.is_open()) {
		...
		while(inpfile.good()) {
			...
			else if(!splitline[0].compare("max_iterations")) {
				maxIterations = atof(splitline[1].c_str());
			}
		}
		inpfile.close();
	}
...
void FluidSimulation::elapseTimeGrid() {
	...
	for (size_t  i_cell = 0; i_cell < gridCells.size(); i_cell++) {
		for (size_t i = 0; i < gridCells[i_cell].size(); i++) {
			...
			vec3 nextPosition = current.position + timeStepSize * (nextCurrentVelocity + timeStepSize * acceleration);
...
int FluidSimulation::drawWaterShape(int n, float xStart, float yStart, float zStart, float xEnd, float yEnd, float zEnd, vec3 velocity){
...
void FluidSimulation::drawTest(int dimension, int version) {
	...
	if (version == 0){
	  ...
	  if (dimensions == 3) {
	    float volume = worldSize * worldSize * worldSize * 0.125;
	  ...
	  vec3 initVelocity1 = vec3(-2.0f, 0.0f, 0.0f);
	  vec3 initVelocity2 = vec3(2.0f, 0.0f, 0.0f);
	  if(dimensions == 3){
